...
class EnViewController: UIViewController {
    ...
    func loadPage() {
        ...
        if self.x == "Closed" {
            ...
            if Tools.setCondtion(FH: 6, LH: 9, FM: 0, LM: 55 , type: now , type2: now) == 0 {
                Tools.ENsetDfference(time2: "9:55AM",TXTTime: self.TXTTime)
                BridgeOn()
            }
            else {
                if Tools.setCondtion(FH: 11, LH: 18, FM: 5, LM: 25 , type: now , type2: now) == 0 {
                    Tools.ENsetDfference(time2: "6:25PM",TXTTime: self.TXTTime)
                    BridgeOn()
                }
                else {
                    if Tools.setCondtion(FH: 19, LH: 2, FM: 35, LM: 55 , type: now ,type2: modifiedDate) == 0 {
                        Tools.ENsetDfference(time2: "2:55AM",TXTTime: self.TXTTime)
                        BridgeOn()
                    }
                    else {
                        ...
                        TXTMess.text = "The Bridge is Closed For Navigation"
    ...
    func BridgeOn () {
        ...
        TXTMess.text = "The Bridge is Working now For Cars"
    ...
    func UpdateUI () {
        ...
        TXTTime.isHidden = false
        TXTMess.text = "The bridge will be closed for emergency form"
        TXTTime.text = "\(mnow)"
        BTNChangeStatue.setBackgroundImage(UIImage(named: "BTNClose"), for: UIControl.State.normal)
        setVideo1()
    }
